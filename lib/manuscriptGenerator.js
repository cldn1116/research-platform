...
function generateManuscript(project, methods, experiments, results) {
  ...
  return {
    ...
    conclusion: buildConclusion(project, included, resultMap),
...
function buildConclusion(project, included, resultMap) {
  const topic = project.research_topic || 'the subject under investigation';
  const n = included.length;
  const withResults = included.filter(e => resultMap?.[e.id]?.raw_text);

  const sentences = [
    `In conclusion, the present study has systematically examined ${topic}.`,
  ];

  if (n > 0) {
    // Name up to 3 experiments for specificity
    const names = included.slice(0, 3).map(e => e.name);
    const nameStr = names.length > 2
      ? `${names[0]}, ${names[1]}, and ${n - 2} further condition${n - 2 > 1 ? 's' : ''}`
      : names.join(' and ');
    sentences.push(
      `Through ${n} controlled experiment${n !== 1 ? 's' : ''} (including ${nameStr}), ` +
      `key parameters governing ${topic} were characterised under defined experimental conditions.`
    );
  }

  // Pull the first key finding from actual result data
  if (withResults.length > 0) {
    const keyExp    = withResults[0];
    const keyText   = formalizeText(resultMap[keyExp.id].raw_text);
    const firstSent = splitSentences(keyText)[0];
    if (firstSent) {
      sentences.push(`Among the principal observations, ${lcFirst(firstSent)}`);
    }
  }

  sentences.push(
    `These findings provide a reproducible experimental basis and suggest productive directions for further investigation into ${topic}.`
  );
