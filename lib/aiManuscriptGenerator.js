...
LANGUAGE: English only. Do not write a single word in Korean, Chinese, or any other language.
If the input contains non-English text, translate it silently before using it.
...
6. Eliminate vague phrases ("the results indicate that", "it was found that", "interestingly").
   State findings directly: "OD600 reached 1.24 at 24 h" not "growth was observed".
7. If growth_curve_params is provided for an experiment, you MUST cite the exact numerical values
   (muMax, lagPhase, maxValue with units) in the formalText paragraph.
   Example: "The maximum specific growth rate (μmax) was 0.42 h⁻¹ with a lag phase of 1.8 h."
8. If method_objective is provided, open the formalText with a brief context sentence referencing
   what was being measured (1 sentence max), then present the data.
9. End formalText with: "Growth kinetics are presented in Figure [N]." only when growth_curve_params
   is present — replace [N] with the experiment's sequential number (1-based).
10. Return ONLY a valid JSON object. No markdown code fences, no text outside the JSON.`;
    ...
    "experiments": [
      {
        ...
        "formalText": "<one formal academic paragraph: [optional 1-sentence method context] + data-specific findings with exact numbers + [Figure N reference if growth_curve_params present]>"
      }
    ]
  },
  "discussion": {
    "overview": "<1–2 sentences — state concretely what was studied and the key quantitative finding, not vague significance claims>",
    "perExperiment": [
      {
        "name": "<experiment name>",
        "interpretation": "<2–3 sentences — data-grounded interpretation; cite specific values from the results; do not invent mechanisms>"
      }
    ],
    "limitations": "<2–3 sentences — specific limitations based on experimental design (sample size, controls, conditions)>",
    "futureDirections": "<2–3 sentences — concrete follow-up experiments arising directly from these specific results>"
  }
}`;
