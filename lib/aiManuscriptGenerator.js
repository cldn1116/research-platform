 ...
 *   computeConclusionHash     → Conclusion cache hash
 *   generateAiConclusion      → Conclusion section (3-part structure)
 */
      ...
      .map(e => ({
        ...
        growth_params: (() => {
          ...
          if (gcd?.version === 2 && gcd.series?.some(s => s.params))
            return gcd.series.filter(s => s.params).map(s => ({ series: s.name, params: s.params }));
          return gcd?.params || null;
        })(),
      }))
...
export async function generateAiSections(project, included, methods, results) {
  ...
  const experiments = withResults.map(e => {
    ...
    if (m) {
      entry.method = m.name;
      // Truncate to first line, max 150 chars to keep prompt concise
      if (m.objective) entry.method_objective = m.objective.split('\n')[0].slice(0, 150);
    }

    // Growth curve parameters — include exact measured values so Claude can cite them
    const gcd = resultMap[e.id]?.growth_curve_data;
    if (gcd?.version === 2 && gcd.series?.some(s => s.params)) {
      // v2 multi-series: pass per-series params
      entry.growth_curve_params = gcd.series
        .filter(s => s.params)
        .map(s => ({ series: s.name, params: s.params }));
    } else if (gcd?.params) {
      // v1 single-series
      entry.growth_curve_params = {
        unit:     gcd.unit || 'OD',
        muMax:    gcd.params.muMax,
        lagPhase: gcd.params.lagPhase,
        maxValue: gcd.params.maxValue,
      };
    }
...
3. If growth_curve_params is an array, cite each series' key metrics with units:
   - biomass (OD/CDW): cite muMax (h⁻¹), lagPhase (h), maxValue
   - ethanol: cite peakConc, peakTime (h), onsetTime if present
   - substrate (glucose/etc.): cite initVal, depletionTime, finalResidual
   - pH: cite minValue, timeToMin (h), maxValue
   If growth_curve_params is an object (v1 format), cite muMax, lagPhase, maxValue with units.
4. If method_objective is given, open formalText with one brief context sentence about what was measured.
5. In discussion perExperiment interpretations use hedged language:
   "may be associated with", "is consistent with", "suggests that", "appears to".
   Do NOT assert strong mechanisms without direct evidence from the provided data.
...
Return ONLY this JSON — no extra keys, no fences:
{
  "results": {
    "experiments": [
      { "id": <integer>, "formalText": "<formal paragraph citing specific values>", "englishTitle": "<short English title ≤60 chars, no Korean>" }
    ]
  },
  "discussion": {
  ...
  // ── Enrich results with experiment names + englishTitle ──────────────────
  ...
  if (Array.isArray(parsed.results.experiments)) {
    parsed.results.experiments = parsed.results.experiments.map(e => ({
      ...
      // Keep englishTitle only if it is non-empty ASCII (no Korean)
      englishTitle: (e.englishTitle && !/[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F]/.test(e.englishTitle))
        ? e.englishTitle
        : null,
    }));
  }
...
/**
 * Compute a deterministic hash of the AI sections used for Conclusion generation.
 * Changes whenever Results or Discussion AI content changes.
 *
 * @param {Object} project
 * @param {Object} manuscript - the manuscript JSONB object
 * @returns {string} hex digest
 */
export function computeConclusionHash(project, manuscript) {
  const payload = {
    title:   (project.title          || '').trim(),
    topic:   (project.research_topic || '').trim(),
    results_texts:    (manuscript?.results_ai?.experiments || [])
      .map(e => (e.formalText || '').slice(0, 400)),
    discussion_overview: (manuscript?.discussion_ai?.overview || '').slice(0, 400),
    discussion_perExp:   (manuscript?.discussion_ai?.perExperiment || [])
      .map(p => (p.interpretation || '').slice(0, 200)),
    discussion_limits:   (manuscript?.discussion_ai?.limitations || '').slice(0, 200),
  };
  return createHash('sha256').update(JSON.stringify(payload)).digest('hex');
}

/**
 * Generate a 3-part AI Conclusion from existing Results + Discussion AI sections.
 * Requires at least results_ai or discussion_ai to exist in manuscript.
 *
 * Structure:
 *   (1) keyResults   — specific quantitative findings with numbers
 *   (2) significance — scientific meaning and interpretation
 *   (3) applications — future directions / practical relevance
 *
 * @param {Object} project
 * @param {Object} manuscript - the manuscript JSONB object (with AI sections)
 * @returns {{ conclusion_ai, conclusionHash }}
 */
export async function generateAiConclusion(project, manuscript) {
  const conclusionHash = computeConclusionHash(project, manuscript);

  // Build compact synthesis from AI sections
  const sections = [];

  if (manuscript?.results_ai?.experiments?.length) {
    const resultsSummary = manuscript.results_ai.experiments
      .map(e => `- [${e.name || `exp ${e.id}`}] ${(e.formalText || '').slice(0, 400)}`)
      .join('\n');
    sections.push(`RESULTS:\n${resultsSummary}`);
  }

  if (manuscript?.discussion_ai?.overview) {
    sections.push(`DISCUSSION OVERVIEW:\n${manuscript.discussion_ai.overview}`);
  }

  if (manuscript?.discussion_ai?.perExperiment?.length) {
    const perExp = manuscript.discussion_ai.perExperiment
      .map(p => `- ${p.name}: ${(p.interpretation || '').slice(0, 200)}`)
      .join('\n');
    sections.push(`DISCUSSION PER-EXPERIMENT:\n${perExp}`);
  }

  if (manuscript?.discussion_ai?.limitations) {
    sections.push(`LIMITATIONS:\n${manuscript.discussion_ai.limitations}`);
  }

  if (sections.length === 0) {
    throw new Error(
      'Conclusion을 생성하려면 먼저 Results & Discussion AI를 생성하세요.'
    );
  }

  const system = `You are writing the Conclusion section of a scientific manuscript for a PhD thesis.
Write in formal academic English.

STRICT RULES:
1. ONLY use information present in the provided results and discussion. Do NOT fabricate data.
2. Cite specific quantitative values from the results (e.g. "μmax of 0.42 h⁻¹", "OD600 reached 1.8").
3. Use hedged language in the significance and applications parts: "suggest", "may", "could", "appear to".
4. Do NOT invent citations, references, author names, or publication details.
5. Keep each part concise: 2–4 sentences.
6. Return ONLY a valid JSON object. No markdown fences, no text outside the JSON.`;

  const userContent = `Write the Conclusion section for this scientific manuscript.

Project title: ${project.title || 'Untitled'}
Research topic: ${project.research_topic || '(not specified)'}

Synthesize ONLY the information below:
${sections.join('\n\n')}

Return this exact JSON — nothing else:
{
  "keyResults": "<2–4 sentences summarising the most important quantitative findings with specific numbers>",
  "significance": "<2–3 sentences on what these findings mean scientifically, using hedged language>",
  "applications": "<2–3 sentences on potential applications or future research directions>"
}`;
  ...
  const raw = (message.content[0]?.text || '').trim();
  console.log('[AI conclusion] stop_reason:', message.stop_reason);
  console.log('[AI conclusion] raw response:\n', raw.slice(0, 400));
  ...
  // Fallback: store raw text in keyResults if parse fails
  if (!parsed?.keyResults) {
    console.warn('[AI conclusion] JSON parse failed — storing raw text as fallback.');
    return {
      conclusion_ai: { text: raw.replace(/^["']|["']$/g, '').trim() || raw },
      conclusionHash,
      _fallback: true,
    };
  }

  console.log('[AI conclusion] parse succeeded.');
  return {
    conclusion_ai: {
