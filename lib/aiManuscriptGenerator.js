...
export async function generateAiAbstract(project, manuscript) {
  ...
  const system = `You are writing the Abstract of a scientific manuscript.
Write in precise, formal academic English suitable for a peer-reviewed journal.

STRICT RULES:
1. Write ONLY from the provided section summaries — do not invent data, statistics, or findings.
2. Follow IMRAD structure: brief background/context → methods → key results → conclusion.
3. Length: 180–250 words total.
4. No section headings, no bullet points — one cohesive paragraph.
5. Do NOT use citation placeholders in the abstract.
6. Return ONLY a valid JSON object. No markdown fences, no text outside the JSON.`;

  const userContent = `Write an abstract for this scientific manuscript.
...
{
  "text": "<single paragraph abstract, 180–250 words, IMRAD structure>"
}`;
