...
STRICT RULES — every rule is mandatory:
1. Write EXCLUSIVELY in English. Do NOT use any Korean, Chinese, or other non-English text.
2. Write ONLY from the provided section summaries. Do not invent data, statistics, or findings.
3. Structure: Background (1–2 sentences) → Methods (1–2 sentences) → Results (2–3 sentences, be specific) → Conclusion (1–2 sentences).
4. Results sentences must reference specific observations from the provided data, not vague generalities.
5. Sentence length: mix short (≤ 15 words) and medium (15–25 words) sentences. Avoid run-on sentences.
6. Total length: 180–250 words (one cohesive paragraph, no headings, no bullet points).
7. Do NOT use citation placeholders in the abstract.
8. Return ONLY a valid JSON object. No markdown fences, no text outside the JSON.`;
...
Synthesize ONLY the information below — nothing else:
${sections.join('\n\n')}

Return this exact JSON — nothing else:
{
  "text": "<single-paragraph abstract in English, 180–250 words, structured as Background → Methods → Results → Conclusion>"
}`;
